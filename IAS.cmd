...
::============================================================================
::
::   IDM Activation Script (IAS) 繁體中文翻譯版本
::
::   Homepages: https://github.com/lstprjct/IDM-Activation-Script
::              https://t.me/ModByPiash/5
::
::   簡體中文版本由 CJH 翻譯：
::              https://github.com/DianNaoTou/IDM-Activation-Script-Chinese
::
::============================================================================
...
set "blank="
set "mas=https://github.com/DianNaoTou/IDM-Activation-Script-Chinese/"
...
if %errorlevel% NEQ 0 (
echo:
echo Null 服務未執行，指令碼可能無法正常運作。
echo:
echo:
echo 說明：%mas%IAS-Help#troubleshoot
echo:
echo:
ping 127.0.0.1 -n 10
)
...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 錯誤：指令碼使用 LF 行尾，或檔案末尾缺少空白行。
echo:
...
%nceline%
echo 不支援目前的系統版本 [%winbuild%].
echo 本專案僅支援 Windows 7/8/8.1/10/11 及其對應的伺服器版本。
goto done2
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 無法在系統中找到 powershell.exe。
goto done2
)
...
%eline%
echo 指令碼是從暫存資料夾啟動的，
echo 這通常是因為直接從壓縮檔內執行指令碼。
echo:
echo 請先解壓縮檔案，再從解壓縮後的資料夾執行指令碼。
goto done2
)
)
...
echo:
echo PowerShell 沒有正常執行。即將結束...
echo 如果您對 Powershell 應用了限制，請撤銷這些變更。
echo:
echo 請參閱說明頁面：%mas%IAS-Help#troubleshoot
goto done2
)
...
%eline%
echo 此指令碼需要系統管理員權限。
echo 請在指令碼上按一下滑鼠右鍵，選擇「以系統管理員身分執行」。
goto done2
)
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "由 Piash 建立｜CJH 簡中翻譯｜DianNaoTou 繁中翻譯"
echo:            ___________________________________________________ 
echo:
echo:               GitHub：
echo:               https://github.com/DianNaoTou/IDM-Activation-Script-Chinese
echo:            ___________________________________________________ 
echo:                                                               
echo:               [1] 啟用
echo:               [2] 凍結試用
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "請使用鍵盤選擇功能 [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%

if %_erl%==6 exit /b
if %_erl%==5 start https://github.com/DianNaoTou/IDM-Activation-Script-Chinese & goto MainMenu
...
echo:
echo 正在將登錄檔備份至 %SystemRoot%\Temp
...
echo %line%
echo:
call :_color %Green% "IDM 啟用／試用狀態已重設完成。"
...
echo %line%
echo:
echo      「啟用」功能可能無法對所有使用者生效，並可能顯示偽造序號提示。
echo:
call :_color2 %_White% "     " %_Green% "建議改用「凍結試用」選項。"
echo %line%
echo:
choice /C:19 /N /M ">    [1] 返回 [9] 繼續啟用 : "
...
echo:
if not exist "%IDMan%" (
call :_color %Red% "IDM [Internet Download Manager] 尚未安裝"
echo 你可以在 https://www.internetdownloadmanager.com/download.html 下載。
goto done
)
...
call :_color %Red% "無法連線至 internetdownloadmanager.com，即將結束..."
goto done
)
call :_color %Gray% "無法以 Ping 檢查 internetdownloadmanager.com"
echo:
)
...
echo 正在檢查系統資訊 - [%regwinos% ^| %fullbuild% ^| %regarch% ^| IDM: %IDMver%]
...
echo:
echo 正在將 CLSID 登錄檔備份至 %SystemRoot%\Temp
...
echo:
if %frz%==0 (
call :_color %Green% "IDM 已成功啟用。"
echo:
call :_color %Gray% "如果出現偽造序號提示，請使用凍結試用選項。"
) else (
call :_color %Green% "已成功凍結 IDM 的 30 天試用期。"
echo:
call :_color %Gray% "如果 IDM 顯示註冊對話方塊，請重新安裝 IDM。"
)
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "正在搜尋 IDM CLSID 登錄檔： $regPath"
	Write-Host
    ...
    foreach ($lockedKey in $lockedKeys) {
        ...
        Write-Output "$leafValue - 找到已鎖定的登錄檔項目"
    }
    ...
    foreach ($key in $filteredKeys) {
        ...
        if (($defaultValue -match "^\d+$") -and ($key.SubKeyCount -eq 0)) {
            ...
            Write-Output "$($key.PSChildName) - 預設值為數字，且沒有子機碼"
            continue
        }
        if (($defaultValue -match "\+|=") -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 預設值符合條件，且沒有子機碼"
            continue
        }
        ...
        if (($versionValue -match "^\d+$") -and ($key.SubKeyCount -eq 1)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在 \Version 中找到數字，且沒有其他子機碼"
            continue
        }
        $keyValues.PSObject.Properties | ForEach-Object {
            if ($_.Name -match "MData|Model|scansk|Therad") {
                $finalValues += $($key.PSChildName)
                Write-Output "$($key.PSChildName) - 找到 MData／Model／scansk／Therad 值"
                continue
            }
        }
        if (($key.ValueCount -eq 0) -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 找到空白子機碼"
            continue
        }
    }
}
...
if ($finalValues -ne $null) {
    Write-Host
    if ($lockKey -ne $null) {
        Write-Host "正在鎖定 IDM CLSID 登錄檔項目..."
    }
    if ($deleteKey -ne $null) {
        Write-Host "正在刪除 IDM CLSID 登錄檔項目..."
    }
    Write-Host
} else {
    Write-Host "找不到 IDM CLSID 登錄檔項目。"
	Exit
}
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 登錄檔項目超過 20 個，將改為刪除而非鎖定..."
	Write-Host
}
    ...
    foreach ($finalValue in $finalValues) {
        ...
        if ($lockKey -ne $null) {
            ...
            try {
                ...
                Write-Host -back 'DarkRed' -fore 'white' "失敗 - $fullPath"
            }
            catch {
                Write-Host "已鎖定 - $fullPath"
            }
        }
